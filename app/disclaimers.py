...
BASE = [
    ...
    # ↓ 2026-09-18 实测补充：收益维度没有证据，这一点必须说在最前面
    # 注意：这些字符串直接渲染成纯文本，**不要写 Markdown 强调语法**（星号会原样显示）
    "【收益维度无证据】核心分 Top-10% 的净选择效应（已扣除全市场等权基准）"
    "仅 +0.038pp/月，95% 区间 [−0.793, +0.929]，跨 0；在现实费率（双边 0.2~0.3%）下"
    "被交易成本吃光（盈亏平衡单边费率仅 0.094%）。"
    "它的证据只在尾部风险维度：未来 60 日跌超 20% 的坏率从 11.14% 降到 4.52%。"
    "请把它当作「避免踩雷的筛选器」，不是收益策略。",
    "数据为离线快照，非实时行情。",
    "股票池不含已退市股票 → 历史坏率是下界，正面结论被高估（负面结论更稳健）。",
    "未做行业/市值中性化：核心分高分组集中在低估值红利（高速/港口/公用事业），是风格暴露而非 alpha。",
    "4 本核心分相对 29 本等权的改进在统计上不显著（置信区间跨 0）。",
    "交易成本：月换手实测 20.2%（不是 100%），双边 0.2~0.3% 时约吃掉 0.04~0.08pp/月。",
